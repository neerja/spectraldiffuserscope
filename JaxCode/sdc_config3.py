...
def forwardmodel3d(x,hfftpad,m):
    ...
    y = abs(torch.fft.ifftshift(torch.fft.ifft2(hfftpad*xfftpad), dim=(-1,-2))) # do convolution
...
def adjointoperation(y,hfftpad,m): 
    ...
    x = abs(torch.fft.ifftshift(torch.fft.ifft2(yfftpad*torch.conj(hfftpad)),dim=(-1,-2))) # apply conv between psf and y
